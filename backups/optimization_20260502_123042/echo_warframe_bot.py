...
def detect_game_state():
    ...
    try:
        ...
        if any(x in text for x in ["navigation", "star chart", "solar system", "select mission"]):
            ...
            # Default to in_mission when text is empty — orbiter always has some UI text
            state = "in_mission"
        else:
...
_bot_typing = False   # set True while bot is sending inputs
...
def basic_combat_loop(duration_minutes=20):
    ...
    while time.time() < end_time:
        ...
        # ── YOUR PLAY STYLE (learned from practice sessions) ──────────────
        ...
        # Constant forward movement (W dominant — 3841 presses per session)
        env = {**os.environ, "DISPLAY": WARFRAME_DISPLAY}
        global _bot_typing
        _bot_typing = True
        subprocess.run(["xdotool", "keydown", "w"], env=env, capture_output=True)
        time.sleep(0.6)
        subprocess.run(["xdotool", "keyup", "w"], env=env, capture_output=True)
        _bot_typing = False

        # Strafe right more than left (d > a from session data)
        if loop_count % 4 == 0:
            subprocess.run(["xdotool", "keydown", "d"], env=env, capture_output=True)
            time.sleep(0.3)
            subprocess.run(["xdotool", "keyup", "d"], env=env, capture_output=True)
        elif loop_count % 8 == 0:
            subprocess.run(["xdotool", "keydown", "a"], env=env, capture_output=True)
            time.sleep(0.2)
            subprocess.run(["xdotool", "keyup", "a"], env=env, capture_output=True)

        # Bullet jump every ~15 seconds (Ctrl+Space pattern)
        if loop_count % 50 == 0:
            subprocess.run(["xdotool", "keydown", "ctrl"], env=env, capture_output=True)
            time.sleep(0.1)
            subprocess.run(["xdotool", "keydown", "space"], env=env, capture_output=True)
            time.sleep(0.15)
            subprocess.run(["xdotool", "keyup", "space"], env=env, capture_output=True)
            subprocess.run(["xdotool", "keyup", "ctrl"], env=env, capture_output=True)
            time.sleep(0.2)

        # Melee every other tick
        if loop_count % 2 == 0:
            key(MELEE, delay=0.1)
