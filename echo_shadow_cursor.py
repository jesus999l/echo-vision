...
class ShadowCursor(Gtk.Window):
    def __init__(self):
        ...
        self.pulse = 0.0
        self.follow_mode = False   # companion mode — trails mouse
        self.follow_offset = (24, 24)  # offset from real cursor
    ...
    def _animate(self):
        ...
        # Follow mouse if in companion mode
        if self.follow_mode:
            try:
                ...
                self.target_x = mx + self.follow_offset[0]
                self.target_y = my + self.follow_offset[1]
                self.visible = True
            except: pass
    ...
    def _handle_command(self, data):
        try:
            ...
            if action == "click":
                ...
                def _set_follow(e=enabled):
                    self.follow_mode = e
                    self.visible = e
                    if not e:
                        self._hide()
                GLib.idle_add(_set_follow)
            elif action == "ai":
