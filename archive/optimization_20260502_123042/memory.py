...
# ── CALENDAR ──────────────────────────────────────────────────────────────────
def _expand_recurring(event, start, end):
...
def add_goal(title, description="", category="personal", target_date=None, priority="normal"):
    now   = time.time()
    color = _GOAL_COLORS.get(category or "personal", "#7c6af7")
    conn  = get_db()
    conn.execute(
        "INSERT INTO goals (title,description,category,target_date,created_at,priority) VALUES (?,?,?,?,?,?)",
